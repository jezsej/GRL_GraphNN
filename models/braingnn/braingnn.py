...
from .bgnncomponents.braingraphconv import MyNNConv


##########################################################################################################################
class Network(torch.nn.Module): #indim, ratio, nclass=2, k=8, R=200
    def __init__(self, cfg, dataloaders = None, K=8):
        '''
        ...
        self.dim5 = 8
        self.k = K
        self.R = self.cfg.models.nroi # regions of interest
        ...
        self.fc3 = torch.nn.Linear(self.dim3, self.nclass)
    ...
    def extract_features(self, data):
        """
        Extract graph-level features from BrainGNN before the final classification layer.
        ...
        """
        x, edge_index, edge_attr, batch, pos, pseudo = data.x, data.edge_index, data.edge_attr, data.batch, data.pos, data.pseudo

        x = self.conv1(x, edge_index, edge_attr, pseudo)
        ...
        x = self.conv2(x, edge_index, edge_attr, pseudo)
        ...
        return x  # shape [batch_size, dim3]
