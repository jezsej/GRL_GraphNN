...
class SpatioTemporalModel(nn.Module, ):
    def __init__(self, cfg, dataloader):
        ...
        dropout_perc = cfg.models.dropout
        pooling = cfg.models.pooling
        activation = cfg.models.activation
        conv_strategy = cfg.models.param_conv_strategy
        sweep_type = cfg.models.sweep_type
        # gat_heads = cfg.model.param_gat_heads
        edge_weights = True
        final_sigmoid = True
        num_nodes = cfg.models.num_nodes
        temporal_embed_size = cfg.models.temporal_embed_size
        threshold = cfg.models.threshold
        batch_size = cfg.dataset.batch_size
        attention_threshold = cfg.models.attentionthreshold
        number_hidden_units = cfg.models.tcn_hidden_units

        # BrainGNN
        self.lamb0 = cfg.models.lamb0
        self.lamb1 = cfg.models.lamb1
        self.lamb2 = cfg.models.lamb2
        self.lamb3 = cfg.models.lamb3
        self.lamb4 = cfg.models.lamb4
        self.lamb5 = cfg.models.lamb5
        self.layer = cfg.models.layer
        self.n_bgnn_layers = cfg.models.n_layers
        self.n_fc_layers = cfg.models.n_fc_layers
        self.n_clustered_communities = cfg.models.n_clustered_communities
        self.bgnn_ratio = cfg.models.bgnnratio
        # fc_dropout = cfg.models.fc_dropout
        ...
        self.dynamic = cfg.models.dynamic
        ...
        if self.conv_strategy == ConvStrategy.TCN_ENTIRE:
            if cfg.models.tcn_hidden_units == 8:
            ...
            tcn_layers = []
            for i in range(cfg.models.tcn_depth):
                if cfg.models.tcn_hidden_units == 8:
                    tcn_layers.append(self.channels_conv * (2 ** i) )
                else:
                    tcn_layers.append(cfg.models.tcn_hidden_units)

            self.temporal_conv1 = TemporalConvNet(1,
                                                 tcn_layers,
                                                 kernel_size=cfg.models.tcn_kernel1,
                                                 dropout=self.dropout,
                                                 norm_strategy=cfg.models.tcn_norm_strategy)
            ...
            self.timepointattention =  TimePointsTransformerModel(d_model=self.num_nodes, nhead=cfg.models.timepoints_attnhead, num_layers=1)

            self.fnc_attention_module = FNCCustomMultiheadAttention(embed_dim= size_per_block, num_heads=cfg.models.fnc_attnhead, q_dim=cfg.models.fnc_embed_dim)
    ...
    def extract_features(self, data):
        """
        Extract graph-level features from dsam before the final classification layer.

        Args:
            data: a PyTorch Geometric Data object with fields:
                time_series, edge_index_tensor, edge_attr_tensor, node_feature, pseudo_torch, batch_tensor

        Returns:
            torch.Tensor: Graph-level feature tensor of shape [num_graphs, dim3]
        """
        device = 'cuda:0' if torch.cuda.is_available() else 'cpu'

        # Temporal TCN
        x = data.time_series.view(-1, 1, self.num_time_length)
        x = self.temporal_conv1(x)
        ...
        # Concatenate all levels to form final output that is comprised of top-k timepoints from each level for each node
